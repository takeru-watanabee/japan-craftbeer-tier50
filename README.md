...
| 時期 | トリガー | 対応 | 自動化 |
|---|---|---|---|
| 3/25 | Japan Great Beer Awards 発表 | A/B 帯補強 | ✅ Issue 自動起票 |
| 4/25 | World Beer Cup 結果発表 | S 帯入れ替え検討、`evidences` 追加 | ✅ Issue 自動起票 |
| 6/1 | Provisional 自動降格チェック | 操業 3 年経過銘柄を通常 Tier に | ✅ Issue 自動起票 + `node scripts/check-provisional.js` の出力同梱 |
| 8/1 | WBA Country Winner ラウンド | 国別 Gold 候補抽出 | ✅ Issue 自動起票 |
| 11/1 | WBA Style Winner / World's Best | 世界 1 位 → 軸1=5 確定、Tier S 候補 | ✅ Issue 自動起票 |

### 🤖 自動 Issue 起票 (GitHub Actions)

`.github/workflows/update-reminder.yml` が上記 5 トリガーで自動的に [Issues](https://github.com/takeru-watanabee/japan-craftbeer-tier50/issues) に Issue を起票します。

各 Issue には:
- 公式ソース URL
- 作業手順チェックリスト
- Claude Code 並列検証プロンプトへのリンク (`prompts/verify-awards.md`)
- 影響予測テーブル

が自動で含まれます。手動実行は [Actions タブ → Update Reminder → Run workflow](https://github.com/takeru-watanabee/japan-craftbeer-tier50/actions) から可能。

### 🍺 ブルワリーからの判定異論

Tier 判定や評価項目に異論がある場合は、[ブルワリー判定への異論・訂正依頼](https://github.com/takeru-watanabee/japan-craftbeer-tier50/issues/new?template=brewery-correction.md) テンプレートから Issue を起票してください。受賞情報は **大会名・年・賞ランク・銘柄名** の 4 要素揃いで提供をお願いします。
